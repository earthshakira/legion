...
func (s *RaftStore) Open(enableSingle bool, localID string) error {
	...
	s.logger = log.New(os.Stderr, "[store "+localID+"] ", log.LstdFlags)
...
// Leader tells if the current RaftNode is leader or not.
func (s *RaftStore) Leader() bool {
	return s.raft.State() == raft.Leader
}

// Get returns the value for the given key.
func (s *RaftStore) Get(key string) ([]byte, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	v, exists := s.m[key]
	return v, exists
...
func (s *RaftStore) Set(key string, value []byte) error {
...
// Remove removes a node, identified by nodeID and located at addr, to this store.
// The node must be ready to respond to Raft communications at that address.
func (s *RaftStore) Remove(nodeID, addr string) error {
	s.logger.Printf("removing node %s at %s", nodeID, addr)
