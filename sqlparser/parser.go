...
func (p *parser) doParse() (query.Query, error) {
	for {
		...
		case stepCreateFields:
			identifier := p.peek()
			if !isIdentifier(identifier) {
				return p.query, fmt.Errorf("at CREATE TABLE: expected at least one field to insert")
			}
			p.query.Fields = append(p.query.Fields, identifier)
			p.pop()
		...
		case stepCreateDataTypes:
			...
			case "INTEGER":
				p.query.DTypes = append(p.query.DTypes, query.Int)
			case "DOUBLE":
				p.query.DTypes = append(p.query.DTypes, query.Double)
			case "STRING":
				p.query.DTypes = append(p.query.DTypes, query.String)
			case "DATETIME":
				p.query.DTypes = append(p.query.DTypes, query.DateTime)
			case "GEO2D":
				p.query.DTypes = append(p.query.DTypes, query.Geo2d)
			default:
				return p.query, fmt.Errorf("at CREATE TABLE: expected data type got " + dtype)
			}
			p.pop()
...
var reservedWords = []string{
	...
	"WHERE", "FROM", "SET", "CREATE TABLE", "BOOL", "STRING", "DATETIME", "GEO2D", "DOUBLE", "INTEGER",
}
