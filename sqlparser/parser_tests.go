...
func ParserTests() {
	ts := []testCase{
		...
		{
			Name: "CREATE TABLE simple query test",
			SQL:  "CREATE TABLE 'xyz' (a BOOL,b INTEGER,c GEO2D)",
			Expected: query.Query{
				Type:      query.Create,
				TableName: "xyz",
				Fields:    []string{"a", "b", "c"},
				DTypes:    []query.DType{query.Bool, query.Int, query.Geo2d},
			},
			Err: nil,
		},
	}
	...
	for _, tc := range ts {
		...
		if tc.Err != nil && err == nil {
			r = fmt.Errorf("Error should have been %v", tc.Err)
		}
		if tc.Err == nil && err != nil {
			r = fmt.Errorf("Error should have been nil but was %v", err)
		}
		if tc.Err != nil && err != nil {
			if !reflect.DeepEqual(tc.Err, err) {
				r = fmt.Errorf("Error %v doesn't match given err %v", err, tc.Err)
			}
		}
		if len(actual) > 0 {
			if !reflect.DeepEqual(tc.Expected, actual[0]) {
				r = fmt.Errorf("Error %v doesn't match given output %v", actual[0], tc.Expected)
			}
	...
	fmt.Println(output.ErrorExamples)
	fmt.Println(output.NoErrorExamples)
}
